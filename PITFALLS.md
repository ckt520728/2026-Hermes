...
- [ ] **Phase 2:加入 multi-agent**:用 coder / researcher / reviewer profile 對應 literature / hypothesis / critic 角色,實現 AI Agent Research Automation skill 的完整框架。2026-05-29 已完成本機 profile + kanban 架構設定,並建立 smoke-test 任務圖;目前卡在 Nous Portal OAuth 未登入,完成 `hermes auth add nous --type oauth` 後即可 unblock + dispatch 驗證。詳見 `MULTI_AGENT.md`
